...
// @ts-ignore
import schoolLunchBento from "./assets/images/school_lunch_bento_1781938858378.jpg";
  ...
  useEffect(() => {
    const initializeAndSync = async () => {
      ...
      const unsubRestaurants = onSnapshot(collection(db, "restaurants"), (snapshot) => {
        ...
        // Map list to guarantee correct image URL for the school lunch
        const finalizedList = restList.map((r) => {
          if (r.id === "rest_school_lunch") {
            return {
              ...r,
              imageUrl: schoolLunchBento
            };
          }
          return r;
        });

        setRestaurants(finalizedList);
      });
